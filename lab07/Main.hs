...
p5:: [(String,Colour)]
p5 = sortBy colorSort [(name p, color p )| p <- people] where colorSort (name1, color1) (name2, color2) = compare color1 color2
--------------------------------------------------------
-- The color and the count of all those with that color

p6:: [(Colour,Int)]
p6 = [(head xs, length xs) | xs <- group(sort [color p | p <- people])]
